...
from okta_cli.formatting import (
    OutputFormatter, UserFormatter, GroupFormatter, ConfigFormatter,
    format_and_output, create_table_from_dict, create_table_from_list
)
from okta_cli.progress import (
    ProgressIndicator, progress_spinner, progress_bar, timed_progress_bar,
    api_call_with_progress, connectivity_test_progress, batch_operation_progress,
    StepProgress
)
...
class TestInteractivePrompts:
    ...
    def test_prompt_for_profile_selection(self, mock_echo, mock_prompt):
        ...
        self.interactive.profile_manager.create_profile("test1", "test1.okta.com", "token1")
        self.interactive.profile_manager.create_profile("test2", "test2.okta.com", "token2")
    ...
    @patch('okta_cli.interactive.click.echo')
    def test_configuration_wizard_success(self, mock_echo, mock_confirm, mock_progressbar):
        """Test successful configuration wizard."""
        mock_confirm.side_effect = [True, True, True]  # Test connectivity, continue, set active
        mock_progressbar.return_value.__enter__.return_value = Mock()
        
        with patch.object(self.interactive, 'prompt_for_profile_name', return_value="test"):
            with patch.object(self.interactive, 'prompt_for_domain', return_value="test.okta.com"):
                with patch.object(self.interactive, 'prompt_for_token', return_value="a" * 25):
                    with patch.object(self.interactive.validator, 'validate_connectivity', return_value=True):
    ...
    @patch('okta_cli.interactive.click.confirm')
    @patch('okta_cli.interactive.click.echo')
    def test_interactive_profile_creation_success(self, mock_echo, mock_confirm, mock_progressbar):
    ...
    def test_show_configuration_status(self, mock_echo):
        ...
        self.interactive.profile_manager.create_profile("test", "test.okta.com", "token123")
...
class TestCompletionProvider:
    ...
    def test_get_profile_names(self):
        ...
        self.provider.profile_manager.create_profile("test1", "test1.okta.com", "token1")
        self.provider.profile_manager.create_profile("test2", "test2.okta.com", "token2")
    ...
    @patch('builtins.open', new_callable=lambda: MagicMock())
    def test_setup_completion_zsh(self, mock_open, mock_makedirs, mock_env_get, mock_echo):
