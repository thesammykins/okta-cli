...
def test_create_group(mock_post, mock_config):
    ...
    result = runner.invoke(
        create_group, ["--name", "NewGroup", "--description", "A new test group"]
    )
    ...
    assert (
        mock_post.call_args[1]["json"]["profile"]["description"] == "A new test group"
    )
...
def test_show_group(mock_get, mock_config):
    ...
    mock_response.json.return_value = {
        "id": "1",
        "profile": {"name": "TestGroup", "description": "A test group"},
    }
    mock_get.return_value = mock_response
...
def test_update_group(mock_put, mock_config):
    ...
    mock_response.json.return_value = {
        "id": "1",
        "profile": {"name": "UpdatedGroup", "description": "An updated test group"},
    }
    ...
    result = runner.invoke(
        update_group,
        ["1", "--name", "UpdatedGroup", "--description", "An updated test group"],
    )
    ...
    assert (
        mock_put.call_args[1]["json"]["profile"]["description"]
        == "An updated test group"
    )
