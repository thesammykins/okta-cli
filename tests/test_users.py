...
from okta_cli.users import list_users, create_user, show_user, suspend_user, unsuspend_user, update_user, assign_app, unassign_app, list_app_assignments, deactivate_user, activate_user, reset_password
...
def test_update_user(mock_post, mock_config):
    ...
    assert mock_post.call_args[1]['json']['profile']['email'] == 'updateduser@example.com'
...
def test_assign_app(mock_post, mock_config):
    ...
    runner = CliRunner()
    result = runner.invoke(assign_app, [
        'user123',
        'app456',
        '--profile-attributes', '{"role": "admin"}'
    ])

    assert result.exit_code == 0
    assert "User assigned to application successfully" in result.output
    mock_post.assert_called_once()
    assert mock_post.call_args[0][0] == 'https://test.okta.com/api/v1/apps/app456/assignments'
