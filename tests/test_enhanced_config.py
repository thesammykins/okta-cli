...
from okta_cli.enhanced_config import (
    ...
    delete_profile, validate_profile, health_check
)
...
def test_environment_config_profile_specific():
    ...
    with patch.dict(os.environ, {
        'OKTA_DEV_DOMAIN': 'dev.okta.com',
        'OKTA_DEV_TOKEN': 'dev-token'
    }):
        env_config = EnvironmentConfig()
...
def test_profile_specific_environment_variables(temp_config_dir):
    ...
    with patch.dict(os.environ, {
        'OKTA_DEV_DOMAIN': 'dev-env.okta.com',
        'OKTA_DEV_TOKEN': 'dev-env-token'
    }):
        with patch('requests.get') as mock_get:
            mock_response = MagicMock()
            mock_response.status_code = 200
...
def test_migrate_legacy_config(temp_config_dir):
    ...
    result = manager.migrate_legacy_config(os.path.join(temp_config_dir, "legacy_config"))
    assert result is True
