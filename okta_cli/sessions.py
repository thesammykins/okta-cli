...
def list_sessions(user_id, profile, limit, output):
    ...
    with progress_spinner("Fetching user sessions from Okta API..."):
        try:
            ...
            response = requests.get(f"https://{domain}/api/v1/sessions", headers=headers, params=params, timeout=30)
            ...
            if output == 'table':
                ...
                for session in sessions_data:
                    formatted_sessions.append({
                        ...
                        'Last Factor Verification': session.get('lastFactorVerification', ''),
                        'Last Password Verification': session.get('lastPasswordVerification', '')
                    })
                
                headers = ['ID', 'User ID', 'Login', 'Created', 'Expires', 'Status', 'Last Factor Verification', 'Last Password Verification']
...
def show_session(user_id, session_id, profile, output):
    ...
    with progress_spinner("Fetching session details..."):
        try:
            response = requests.get(f"https://{domain}/api/v1/users/{user_id}/sessions/{session_id}", headers=headers, timeout=30)
        except requests.exceptions.RequestException as e:
            handle_network_error(e)
        ...
        try:
            ...
            if output == 'table':
                ...
                formatted_session = {
                    ...
                    'Last Factor Verification': session_data.get('lastFactorVerification', ''),
                    'Last Password Verification': session_data.get('lastPasswordVerification', ''),
                    'Authentication Method': session_data.get('authenticationMethod', ''),
...
def clear_sessions(user_id, profile, oauth_only, force):
    ...
    if not force:
        ...
        if not click.confirm(f"Are you sure you want to clear {session_type} for user '{user_id}'?"):
    ...
    with progress_spinner("Clearing user sessions..."):
        try:
            response = requests.delete(f"https://{domain}/api/v1/users/{user_id}/sessions", headers=headers, params=params, timeout=30)
        except requests.exceptions.RequestException as e:
            handle_network_error(e)

    if response.status_code == 204:
        session_type = "OAuth sessions" if oauth_only else "all sessions"
        click.echo(click.style(f"✅ {session_type.capitalize()} cleared successfully!", fg='green'))
    else:
        handle_api_error(response)
...
def extend_session(user_id, session_id, profile, output):
    ...
    with progress_spinner("Extending user session..."):
        try:
            ...
            response = requests.post(f"https://{domain}/api/v1/sessions/{session_id}/lifecycle/refresh", headers=headers, timeout=30)
        except requests.exceptions.RequestException as e:
            handle_network_error(e)
...
@click.option('--profile', default=None, help='The profile to use.')
@click.option('--days', type=int, default=7, help='Number of days to look back for session statistics.')
@create_output_option()
@with_error_handling
    ...
    if response.status_code == 200:
        try:
            ...
            stats = {
                ...
                'unique_users': len(set(log.get('actor', {}).get('id', '') for log in logs_data)),
                'date_range': f"{start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')}",
                'average_sessions_per_day': round(len(logs_data) / days, 2) if days > 0 else 0
            }
            ...
            if output == 'table':
                formatted_stats = {
                    ...
                    'Authentication Methods': ', '.join([f"{k}: {v}" for k, v in auth_methods.items()])
                }
...
def active_sessions(profile, limit, output):
    ...
    with progress_spinner("Fetching active sessions..."):
        try:
            # Get active users
            users_response = requests.get(f"https://{domain}/api/v1/users", headers=headers, params=params, timeout=30)
            ...
            for user in users_data[:20]:
                ...
                if user_id:
                    try:
                        sessions_response = requests.get(f"https://{domain}/api/v1/users/{user_id}/sessions", headers=headers, timeout=10)
    ...
    if output == 'table':
        ...
        headers = ['User Login', 'Session ID', 'Created', 'Expires', 'Status', 'Last Activity']
        format_and_output(formatted_sessions, output, 'generic', headers)
    else:
