...
def get_effective_config(profile: str = None) -> Tuple[str, str]:
    ...
    try:
        ...
        raise ConfigurationError(f"Profile '{profile}' not found and no environment configuration available")
...
def health_check(domain: str, token: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            result["errors"].append(f"Authentication failed: HTTP {response.status_code}")
