...
def validate_okta_domain(domain: str) -> str:
    ...
    if not re.match(okta_pattern, domain):
        raise ValidationError(f"Invalid Okta domain format: {domain}. Expected format: your-domain.okta.com")
...
def handle_api_error(response: requests.Response) -> None:
    ...
    error_message = error_messages.get(status_code, f"HTTP {status_code}: {error_summary}")
...
def handle_network_error(error: requests.exceptions.RequestException) -> None:
    ...
    if isinstance(error, requests.exceptions.ConnectTimeout):
        raise APIError("Connection timeout: Unable to connect to Okta. Please check your internet connection.")
    elif isinstance(error, requests.exceptions.ReadTimeout):
        raise APIError("Request timeout: The request took too long to complete. Please try again.")
    elif isinstance(error, requests.exceptions.ConnectionError):
        raise APIError("Connection failed: Unable to connect to Okta. Please check your internet connection and domain configuration.")
...
def handle_config_error(profile: str) -> None:
    ...
    """
    raise ConfigurationError(f"Profile '{profile}' not found. Please run 'okta-cli configure --profile {profile}' to set up the profile.")
...
def handle_corrupted_config() -> None:
    ...
    Raises:
        ConfigurationError: With appropriate error message
    """
    raise ConfigurationError("Configuration file is corrupted. Please run 'okta-cli configure' to recreate it.")
