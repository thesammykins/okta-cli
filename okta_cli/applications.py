...
@click.option('--limit', type=int, default=20, help='Number of applications to retrieve.')
...
def list_applications(profile, limit, filter, output):
    ...
    with progress_spinner("Fetching applications from Okta API..."):
        try:
            response = requests.get(f"https://{domain}/api/v1/apps", headers=headers, params=params, timeout=30)
...
def show_application(app_id, profile, output):
    ...
    with progress_spinner("Fetching application details..."):
        try:
            response = requests.get(f"https://{domain}/api/v1/apps/{app_id}", headers=headers, timeout=30)
        except requests.exceptions.RequestException as e:
            handle_network_error(e)
...
@click.option('--sign-on-mode', type=click.Choice(['BOOKMARK', 'BASIC_AUTH', 'BROWSER_PLUGIN', 'SECURE_PASSWORD_STORE', 'SAML_2_0', 'WS_FEDERATION', 'AUTO_LOGIN', 'OPENID_CONNECT']), default='BOOKMARK', help='Sign-on mode.')
@click.option('--profile', default=None, help='The profile to use.')
@create_output_option()
@with_error_handling
def create_application(name, label, sign_on_mode, profile, output):
    ...
    with progress_spinner("Creating application in Okta..."):
        try:
            response = requests.post(f"https://{domain}/api/v1/apps", headers=headers, json=data, timeout=30)
        except requests.exceptions.RequestException as e:
            handle_network_error(e)
...
@click.option('--status', type=click.Choice(['ACTIVE', 'INACTIVE']), help='Application status.')
@click.option('--profile', default=None, help='The profile to use.')
@create_output_option()
@with_error_handling
def update_application(app_id, name, label, status, profile, output):
    ...
    if not data:
        click.echo("No update parameters provided. Please provide at least one of --name, --label, or --status.")
        return

    with progress_spinner("Updating application in Okta..."):
        try:
            response = requests.put(f"https://{domain}/api/v1/apps/{app_id}", headers=headers, json=data, timeout=30)
        except requests.exceptions.RequestException as e:
            handle_network_error(e)
...
def delete_application(app_id, profile, force):
    ...
    if not force:
        if not click.confirm(f"Are you sure you want to delete application '{app_id}'?"):
    ...
    with progress_spinner("Deleting application from Okta..."):
        try:
            response = requests.delete(f"https://{domain}/api/v1/apps/{app_id}", headers=headers, timeout=30)
        except requests.exceptions.RequestException as e:
            handle_network_error(e)
...
def activate_application(app_id, profile):
    ...
    with progress_spinner("Activating application in Okta..."):
        try:
            response = requests.post(f"https://{domain}/api/v1/apps/{app_id}/lifecycle/activate", headers=headers, timeout=30)
        except requests.exceptions.RequestException as e:
            handle_network_error(e)
...
def deactivate_application(app_id, profile):
    ...
    with progress_spinner("Deactivating application in Okta..."):
        try:
            response = requests.post(f"https://{domain}/api/v1/apps/{app_id}/lifecycle/deactivate", headers=headers, timeout=30)
        except requests.exceptions.RequestException as e:
            handle_network_error(e)
...
def list_app_users(app_id, profile, limit, output):
    ...
    with progress_spinner("Fetching application users from Okta API..."):
        try:
            response = requests.get(f"https://{domain}/api/v1/apps/{app_id}/users", headers=headers, params=params, timeout=30)
        except requests.exceptions.RequestException as e:
            handle_network_error(e)
...
def list_app_groups(app_id, profile, limit, output):
    ...
    with progress_spinner("Fetching application groups from Okta API..."):
        try:
            response = requests.get(f"https://{domain}/api/v1/apps/{app_id}/groups", headers=headers, params=params, timeout=30)
        except requests.exceptions.RequestException as e:
            handle_network_error(e)
