...
@click.option(
    "--limit", type=int, default=20, help="Number of authorization servers to retrieve."
)
    ...
    with progress_spinner("Fetching authorization servers from Okta API..."):
        try:
            response = requests.get(
                f"https://{domain}/api/v1/authorizationServers",
                headers=headers,
                params=params,
                timeout=30,
            )
                    ...
                    formatted_servers.append(
                        {
                            ...
                            "Audience": (
                                server.get("audiences", [""])[0]
                                if server.get("audiences")
                                else ""
                            ),
...
def show_authorization_server(server_id, profile, output):
    ...
    with progress_spinner("Fetching authorization server details..."):
        try:
            response = requests.get(
                f"https://{domain}/api/v1/authorizationServers/{server_id}",
                headers=headers,
                timeout=30,
            )
        except requests.exceptions.RequestException as e:
...
def create_authorization_server(name, description, audience, profile, output):
    ...
    data = {"name": name, "audiences": [audience]}
    ...
    if response.status_code == 201:
        try:
            ...
            click.echo(
                click.style("✅ Authorization server created successfully!", fg="green")
            )
...
def update_authorization_server(
    ...
    if not any([name, description is not None, audience]):
        click.echo(
            "No update parameters provided. Please provide at least one of --name, --description, or --audience."
        )
        return
    ...
    if response.status_code == 200:
        try:
            ...
            click.echo(
                click.style("✅ Authorization server updated successfully!", fg="green")
            )
...
def delete_authorization_server(server_id, profile, force):
    ...
    if not force:
        if not click.confirm(
            f"Are you sure you want to delete authorization server '{server_id}'?"
        ):
    ...
    if response.status_code == 204:
        click.echo(
            click.style("✅ Authorization server deleted successfully!", fg="green")
        )
    else:
        handle_api_error(response)
...
def activate_authorization_server(server_id, profile):
    ...
    with progress_spinner("Activating authorization server..."):
        try:
            response = requests.post(
                f"https://{domain}/api/v1/authorizationServers/{server_id}/lifecycle/activate",
                headers=headers,
                timeout=30,
            )
        except requests.exceptions.RequestException as e:
    ...
    if response.status_code == 200:
        click.echo(
            click.style("✅ Authorization server activated successfully!", fg="green")
        )
    else:
        handle_api_error(response)
...
def deactivate_authorization_server(server_id, profile):
    ...
    with progress_spinner("Deactivating authorization server..."):
        try:
            response = requests.post(
                f"https://{domain}/api/v1/authorizationServers/{server_id}/lifecycle/deactivate",
                headers=headers,
                timeout=30,
            )
        except requests.exceptions.RequestException as e:
    ...
    if response.status_code == 200:
        click.echo(
            click.style("✅ Authorization server deactivated successfully!", fg="green")
        )
    else:
        handle_api_error(response)
...
def list_scopes(server_id, profile, limit, output):
    ...
    with progress_spinner("Fetching scopes from authorization server..."):
        try:
            response = requests.get(
                f"https://{domain}/api/v1/authorizationServers/{server_id}/scopes",
                headers=headers,
                params=params,
                timeout=30,
            )
...
@click.option(
    "--consent",
    type=click.Choice(["REQUIRED", "OPTIONAL", "IMPLICIT"]),
    default="REQUIRED",
    help="Consent type.",
)
@click.option("--profile", default=None, help="The profile to use.")
