...
from .enhanced_config import ProfileManager, ConfigValidator, EnvironmentConfig, get_effective_config
...
class InteractivePrompts:
    ...
    def prompt_for_profile_name(self, default_name: str = None) -> str:
        ...
        while True:
            profile_name = click.prompt(
                "Profile name",
                default=default_name or "default",
                show_default=True
            )
    ...
    def configuration_wizard(self) -> bool:
        ...
        if click.confirm("\nTest connectivity to Okta API?", default=True):
            ...
            with click.progressbar(
                ...
                connectivity_result = self.validator.validate_connectivity(domain, token)
        ...
        try:
            if profile_name in profiles:
                ...
                click.echo(click.style(f"✅ Profile '{profile_name}' updated successfully!", fg='green'))
            else:
                self.profile_manager.create_profile(profile_name, domain, token)
                click.echo(click.style(f"✅ Profile '{profile_name}' created successfully!", fg='green'))
    ...
    def interactive_profile_creation(self) -> bool:
        ...
        profiles = self.profile_manager.list_profiles()
        if profile_name in profiles:
            click.echo(click.style(f"⚠️  Profile '{profile_name}' already exists.", fg='yellow'))
        ...
        try:
            ...
            click.echo(click.style(f"✅ Profile '{profile_name}' {action} successfully!", fg='green'))
    ...
    def show_configuration_status(self) -> None:
        ...
        if env_domain or env_token:
            ...
            if self.env_config.has_complete_config():
                click.echo(click.style("  ✅ Complete configuration via environment", fg='green'))
            else:
                click.echo(click.style("  ⚠️  Incomplete environment configuration", fg='yellow'))
            click.echo()
        ...
        if profiles:
            ...
            for name, profile_data in profiles.items():
                ...
                click.echo(f"    Created: {time.ctime(profile_data.get('created_at', 0))}")
            click.echo()
        else:
        ...
        try:
            ...
            click.echo(click.style("⚙️  Effective Configuration:", fg='blue', bold=True))
            click.echo(click.style(f"  ❌ {e}", fg='red'))
            click.echo(click.style("  💡 Run 'okta-cli config wizard' to set up", fg='yellow'))
