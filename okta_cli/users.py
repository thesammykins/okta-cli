...
from .errors import (
    ...
    validate_email, validate_login, validate_name
)
...
def list_users(profile, output):
    ...
    with progress_spinner("Fetching users from Okta API..."):
        try:
            response = requests.get(f"https://{domain}/api/v1/users", headers=headers, timeout=30)
...
def create_user(first_name, last_name, email, login, profile, output):
    ...
    with progress_spinner("Creating user in Okta..."):
        try:
            response = requests.post(f"https://{domain}/api/v1/users?activate=false", headers=headers, json=data, timeout=30)
        except requests.exceptions.RequestException as e:
            handle_network_error(e)
...
def assign_app(user_identifier, app_identifier, profile_attributes, profile):
    ...
    response = requests.post(f"https://{domain}/api/v1/apps/{app_id}/users", headers=headers, json=data)
...
def unassign_app(user_id, app_id, profile):
    ...
    response = requests.delete(f"https://{domain}/api/v1/apps/{app_id}/users/{user_id}", headers=headers)
...
def list_app_assignments(app_id, profile):
    ...
    response = requests.get(f"https://{domain}/api/v1/apps/{app_id}/users", headers=headers)
