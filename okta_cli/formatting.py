    ...
    def _format_table(self, data: Any, headers: Optional[List[str]] = None) -> str:
        ...
        if isinstance(data, dict):
            ...
            if isinstance(data[0], dict):
                ...
                if headers:
                    return tabulate(
                        [[item] for item in data], headers=headers, tablefmt="grid"
                    )
                else:
...
class ConfigFormatter:
    ...
    def format_profile_list(
        self, profiles: Dict[str, Any], active_profile: str = None
    ) -> str:
    ...
    def format_profile_detail(
        self, profile_name: str, profile_data: Dict[str, Any]
    ) -> str:
        """Format detailed profile information."""
        created_time = datetime.fromtimestamp(
            profile_data.get("created_at", 0)
        ).strftime("%Y-%m-%d %H:%M:%S")
        last_used_time = datetime.fromtimestamp(
            profile_data.get("last_used", 0)
        ).strftime("%Y-%m-%d %H:%M:%S")
    ...
    def format_health_check(self, health_data: Dict[str, Any]) -> str:
        ...
        health_info = {
            ...
            "Connectivity": (
                "✅ Connected" if health_data.get("connectivity") else "❌ Failed"
            ),
            "Authentication": (
                "✅ Authenticated" if health_data.get("authentication") else "❌ Failed"
            ),
            "Response Time": (
                f"{health_data.get('response_time', 0):.2f}s"
...
def create_table_from_list(
    data: List[Dict[str, Any]], headers: List[str] = None, title: str = None
) -> str:
    """
