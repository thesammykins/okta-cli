...
def resolve_user_id(user_identifier: str, domain: str, token: str) -> Optional[str]:
    ...
    try:
        response = requests.get(f"https://{domain}/api/v1/users/{user_identifier}", headers=headers, timeout=30)
    ...
    try:
        response = requests.get(f"https://{domain}/api/v1/users", headers=headers, params={"search": f'profile.email eq "{user_identifier}" or profile.login eq "{user_identifier}"'}, timeout=30)
        if response.status_code == 200:
            users = response.json()
            if users:
                return users[0]['id']
...
def resolve_group_id(group_identifier: str, domain: str, token: str) -> Optional[str]:
    ...
    try:
        response = requests.get(f"https://{domain}/api/v1/groups", headers=headers, params={"search": f'profile.name eq "{group_identifier}"'}, timeout=30)
        if response.status_code == 200:
...
def resolve_app_id(app_identifier: str, domain: str, token: str) -> Optional[str]:
    ...
    try:
        response = requests.get(f"https://{domain}/api/v1/apps", headers=headers, timeout=30)
        if response.status_code == 200:
            apps = response.json()
            for app in apps:
                if app.get('name') == app_identifier or app.get('label') == app_identifier:
                    return app['id']
    except requests.exceptions.RequestException:
        pass
...
def get_user_by_identifier(user_identifier: str, domain: str, token: str) -> Optional[Dict[str, Any]]:
    """
...
def get_group_by_identifier(group_identifier: str, domain: str, token: str) -> Optional[Dict[str, Any]]:
    """
    ...
    if is_okta_id(group_identifier):
        try:
            response = requests.get(f"https://{domain}/api/v1/groups/{group_identifier}", headers=headers, timeout=30)
            if response.status_code == 200:
                return response.json()
        except requests.exceptions.RequestException:
            pass
...
def get_app_by_identifier(app_identifier: str, domain: str, token: str) -> Optional[Dict[str, Any]]:
    """
    ...
    if is_okta_id(app_identifier):
        try:
            response = requests.get(f"https://{domain}/api/v1/apps/{app_identifier}", headers=headers, timeout=30)
            if response.status_code == 200:
                return response.json()
        except requests.exceptions.RequestException:
            pass
