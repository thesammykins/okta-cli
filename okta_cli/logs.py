...
@click.option('--limit', type=int, default=100, help='Number of log events to retrieve.')
...
def list_logs(profile, limit, since, until, filter, q, output):
    ...
    with progress_spinner("Fetching event logs from Okta API..."):
        try:
            response = requests.get(f"https://{domain}/api/v1/logs", headers=headers, params=params, timeout=30)
            ...
            if output == 'table':
                ...
                for log in logs_data:
                    ...
                    formatted_logs.append({
                        ...
                        'Actor': actor.get('displayName', actor.get('alternateId', '')),
                        'Target': target.get('displayName', target.get('alternateId', '')),
                        'Client': safe_nested_get(log, 'client', 'userAgent', 'browser'),
                        'Outcome': safe_nested_get(log, 'outcome', 'result'),
                        'City': safe_nested_get(log, 'client', 'geographicalContext', 'city'),
                        'Country': safe_nested_get(log, 'client', 'geographicalContext', 'country')
                    })
                
                headers = ['UUID', 'Published', 'Event Type', 'Actor', 'Target', 'Client', 'Outcome', 'City', 'Country']
...
def show_log(log_id, profile, output):
    ...
    with progress_spinner("Fetching log event details..."):
        try:
            response = requests.get(f"https://{domain}/api/v1/logs/{log_id}", headers=headers, timeout=30)
        except requests.exceptions.RequestException as e:
            handle_network_error(e)
    ...
    if response.status_code == 200:
        try:
            ...
            if not isinstance(log_data, dict):
                raise ValueError(f"Expected log object but got {type(log_data).__name__}")
            
            if output == 'table':
                # Format for table display
                actor = log_data.get('actor', {})
                target = log_data.get('target', [{}])[0] if log_data.get('target') else {}
                ...
                formatted_log = {
                    ...
                    'Request ID': safe_nested_get(log_data, 'request', 'ipChain', 0, 'ip') if log_data.get('request', {}).get('ipChain') else '',
...
@click.option('--outcome', type=click.Choice(['SUCCESS', 'FAILURE', 'SKIPPED', 'ALLOW', 'DENY', 'CHALLENGE', 'UNKNOWN']), help='Filter by outcome.')
...
def search_logs(profile, event_type, actor, target, outcome, days, limit, output):
    ...
    if actor:
        filters.append(f'actor.alternateId eq "{actor}" or actor.displayName eq "{actor}"')
    if target:
        filters.append(f'target.alternateId eq "{target}" or target.displayName eq "{target}"')
    if outcome:
        ...
        try:
            ...
            if output == 'table':
                ...
                for log in logs_data:
                    ...
                    target_info = log.get('target', [{}])[0] if log.get('target') else {}
                    
                    formatted_logs.append({
                        'Published': log.get('published', ''),
                        'Event Type': log.get('eventType', ''),
                        'Actor': actor_info.get('displayName', actor_info.get('alternateId', '')),
                        'Target': target_info.get('displayName', target_info.get('alternateId', '')),
                        'Outcome': safe_nested_get(log, 'outcome', 'result'),
                ...
                headers = ['Published', 'Event Type', 'Actor', 'Target', 'Outcome', 'Client IP', 'City', 'Country']
                format_and_output(formatted_logs, output, 'generic', headers)
            else:
                format_and_output(logs_data, output, 'generic')
    ...
    if response.status_code == 200:
        try:
            ...
            for log in logs_data:
                ...
                # Clients
                client = safe_nested_get(log, 'client', 'userAgent', 'browser', default='Unknown')
                clients[client] = clients.get(client, 0) + 1
                
                # Countries
                country = safe_nested_get(log, 'client', 'geographicalContext', 'country', default='Unknown')
                countries[country] = countries.get(country, 0) + 1
            
            # Sort by frequency
            top_event_types = sorted(event_types.items(), key=lambda x: x[1], reverse=True)[:10]
            top_outcomes = sorted(outcomes.items(), key=lambda x: x[1], reverse=True)[:10]
            ...
            if output == 'table':
                formatted_stats = {
                    ...
                    'Top Event Types': ', '.join([f"{k}: {v}" for k, v in list(stats['top_event_types'].items())[:5]]),
                    'Outcomes': ', '.join([f"{k}: {v}" for k, v in stats['outcomes'].items()]),
                    'Top Actors': ', '.join([f"{k}: {v}" for k, v in list(stats['top_actors'].items())[:5]]),
                    'Top Countries': ', '.join([f"{k}: {v}" for k, v in list(stats['top_countries'].items())[:5]])
                }
...
@click.option('--profile', default=None, help='The profile to use.')
@click.option('--days', type=int, default=7, help='Number of days to look back.')
@click.option('--limit', type=int, default=50, help='Number of failed login attempts to retrieve.')
@create_output_option()
@with_error_handling
def failed_logins(profile, days, limit, output):
    ...
    with progress_spinner(f"Fetching failed login attempts for the last {days} days..."):
        try:
            response = requests.get(f"https://{domain}/api/v1/logs", headers=headers, params=params, timeout=30)
        except requests.exceptions.RequestException as e:
            handle_network_error(e)
            ...
            if output == 'table':
                ...
                for log in logs_data:
                    ...
                    formatted_logs.append({
                        'Published': log.get('published', ''),
                        'Event Type': log.get('eventType', ''),
                        'User': actor.get('displayName', actor.get('alternateId', '')),
                        'Client IP': client.get('ipAddress', ''),
                        'User Agent': safe_nested_get(client, 'userAgent', 'rawUserAgent'),
                        'Outcome Reason': safe_nested_get(log, 'outcome', 'reason'),
                        'City': safe_nested_get(client, 'geographicalContext', 'city'),
                        'Country': safe_nested_get(client, 'geographicalContext', 'country')
                    })
                
                headers = ['Published', 'Event Type', 'User', 'Client IP', 'User Agent', 'Outcome Reason', 'City', 'Country']
                format_and_output(formatted_logs, output, 'generic', headers)
            else:
                format_and_output(logs_data, output, 'generic')
...
@click.option('--profile', default=None, help='The profile to use.')
@click.option('--days', type=int, default=7, help='Number of days to look back.')
@click.option('--limit', type=int, default=50, help='Number of suspicious activities to retrieve.')
@create_output_option()
@with_error_handling
def suspicious_activities(profile, days, limit, output):
    ...
    with progress_spinner(f"Fetching suspicious activities for the last {days} days..."):
        try:
            response = requests.get(f"https://{domain}/api/v1/logs", headers=headers, params=params, timeout=30)
        except requests.exceptions.RequestException as e:
            handle_network_error(e)
    ...
    if response.status_code == 200:
        try:
            ...
            if output == 'table':
                ...
                headers = ['Published', 'Event Type', 'User', 'Client IP', 'Outcome', 'Outcome Reason', 'City', 'Country', 'Display Message']
                format_and_output(formatted_logs, output, 'generic', headers)
            else:
                format_and_output(logs_data, output, 'generic')
