...
def create_group(name, description, profile):
    ...
    response = requests.post(f"https://{domain}/api/v1/groups", headers=headers, json=data)
...
@click.option('--profile', default='default', help='The profile to use.')
def show_group(group_id, profile):
    ...
    response = requests.get(f"https://{domain}/api/v1/groups/{group_id}", headers=headers)
...
def update_group(group_id, name, description, profile):
    ...
    if description: profile_data["description"] = description

    if not profile_data:
        click.echo("No update parameters provided. Please provide at least one of --name or --description.")
        return

    data = {"profile": profile_data}

    response = requests.put(f"https://{domain}/api/v1/groups/{group_id}", headers=headers, json=data)
...
def delete_group(group_id, profile):
    ...
    response = requests.delete(f"https://{domain}/api/v1/groups/{group_id}", headers=headers)
...
@click.option('--profile', default='default', help='The profile to use.')
def add_user_to_group(group_id, user_id, profile):
    """Adds a user to a group in Okta."""
    cfg = config.get_config()
    if not cfg.has_section(profile):
        click.echo(f"Profile '{profile}' not found. Please run `okta-cli configure`.")
        return
    ...
    response = requests.put(f"https://{domain}/api/v1/groups/{group_id}/users/{user_id}", headers=headers)
...
def remove_user_from_group(group_id, user_id, profile):
    ...
    response = requests.delete(f"https://{domain}/api/v1/groups/{group_id}/users/{user_id}", headers=headers)
...
def list_group_members(group_id, profile):
    ...
    response = requests.get(f"https://{domain}/api/v1/groups/{group_id}/users", headers=headers)
